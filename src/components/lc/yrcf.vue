<template>
  <div class="yrcf" ref="content">
    <div class="yrcf-content">
      <div class="list-box" v-for="(item,index) in yrcfData">
...
<script>
  ...
  export default{
    ...
    created() {
      this.$http.get('/api/yrcfData').then((response) => {
        if ( response.body.errno === ERR_OK) {
          this.yrcfData = response.body.data.RETURN;
          this.$nextTick(() => {
            this._initScroll();
          });
        }
