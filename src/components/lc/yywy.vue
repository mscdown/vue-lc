<template>
  <div class="yywy" ref="content">
    <div class="yywy-content">
      <div class="list-box" v-for="(item,index) in yywyData">
        <a href="../detail" class="goseach">
          <div class="left">
            <img  src="../../assets/yue-big.png" alt="">
          </div>
          <div class="right">
            <ul>
...
<script>
  ...
  export default{
    ...
    created() {
      this.$http.get('/api/yywyData').then((response) => {
        if ( response.body.errno === ERR_OK) {
          this.yywyData = response.body.data.RETURN;
          this.$nextTick(() => {
            this._initScroll();
          });
        }
